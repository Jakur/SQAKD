...
from models.custom_models_vgg import vgg13_bn_fp
...
if classes == 10:
    ...
    raise NotImplementedError(f"Unknown number of classes: {classes}")
# classes = 100
...
#  L ship/
#  L truck/
path_to_train = f"/home/justin/Code/Oakland/distill/data/{cifar_str}/train"
path_to_test = f"/home/justin/Code/Oakland/distill/data/{cifar_str}/test"
...
class SimSiamModel(BenchmarkModule):
    def __init__(self, dataloader_kNN, num_classes, use_resnet=True):
        super().__init__(dataloader_kNN, num_classes)
        # create a ResNet backbone and remove the classification head
        if use_resnet:
            resnet = ResNetGenerator("resnet-18")
            ...
            print("Using VGG-13")
            self.backbone = vgg13_bn_fp(dotdict({"num_classes": num_classes}))
...
class FastSiamModel(SimSiamModel):
    def __init__(self, dataloader_kNN, num_classes, use_resnet=True):
        super().__init__(dataloader_kNN, num_classes, use_resnet=use_resnet)
...
use_resnet = True
if run_linear:
    # checkpoint = "benchmark_logs/cifar10/version_1/BYOL/checkpoints/epoch=199-step=78000.ckpt"
    checkpoint = "benchmark_logs/cifar10/version_2/FastSiam/checkpoints/epoch=199-step=78000.ckpt"
    # checkpoint = "benchmark_logs/cifar10/version_3/FastSiam/checkpoints/epoch=199-step=78000.ckpt"
    # dataset_train_ssl = create_dataset_train_ssl(BYOLModel)
    dataset_train_ssl = create_dataset_train_ssl(models[0])
    dataloader_train_ssl, dataloader_train_kNN, dataloader_test = get_data_loaders(
        batch_size=batch_size, dataset_train_ssl=dataset_train_ssl
    )
    byol = models[0].load_from_checkpoint(checkpoint, dataloader_kNN=dataloader_train_kNN, num_classes=classes, use_resnet=use_resnet)
    from linear_classify import linear_eval, MyLinearClassifier
    from linear_head import train_linear
    linear_eval(byol, dataloader_train_kNN, dataloader_test, os.path.join(logs_root_dir, cifar_str), batch_size, "cuda", 1,
                32, classes, freeze_model=False)
    # score = train_linear(MyLinearClassifier(byol, 128, 512, 100, freeze_model=True).cuda(), 
    #              dataloader_train_kNN, dataloader_test, "cuda", normalize=False)
    # print(f"Accuracy: {100.0 * score:.2f}%")
    exit()
    assert(False)
...
for BenchmarkModel in models:
    ...
    for seed in range(n_runs):
        ...
        benchmark_model = BenchmarkModel(dataloader_train_kNN, classes, use_resnet=use_resnet)

        # Save logs to: {CWD}/benchmark_logs/cifar10/{experiment_version}/{model_name}/
        # If multiple runs are specified a subdirectory for each run is created.
        sub_dir = model_name if n_runs <= 1 else f"{model_name}/run{seed}"
        logger = TensorBoardLogger(
            save_dir=os.path.join(logs_root_dir, cifar_str),
            name="",
            sub_dir=sub_dir,
